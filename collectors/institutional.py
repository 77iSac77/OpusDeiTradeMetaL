...
class InstitutionalCollector:
    ...
    def __init__(self):
        ...
        # EndereÃ§os de exchanges conhecidos
        ...
        try:
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    # Mapear cÃ³digos CFTC para nossos tickers
                    cftc_map = {
                        "GOLD": "XAU",
                        "SILVER": "XAG",
                        "PLATINUM": "XPT",
                    ...
                    for line in lines[1:]:  # Pular header
                        try:
                            ...
                            # Verificar se Ã© um metal que monitoramos
                            metal_code = None
                            ...
                            # Estrutura varia, isso Ã© simplificado
    ...
    async def fetch_etf_holdings(self, etf_symbol: str) -> Optional[ETFFlow]:
        ...
        Args:
            etf_symbol: SÃ­mbolo do ETF (GLD, SLV, etc)
        ...
        try:
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    # Isso Ã© uma aproximaÃ§Ã£o
                    # Dados reais viriam do site do ETF
                    etf_flow = ETFFlow(
                        etf_symbol=etf_symbol,
                        metal=metal,
                        holdings_tons=0,  # Precisaria de dados especÃ­ficos
    ...
    async def fetch_gld_holdings(self) -> Optional[ETFFlow]:
        """
        Coleta holdings especÃ­ficos do GLD (SPDR Gold).
    ...
    async def fetch_token_transfers(self, token: str, 
        ...
        """
        Coleta transferÃªncias de token via Etherscan.
        
        Args:
            token: Nome do token (PAXG, XAUT)
            blocks_back: Quantos blocos para trÃ¡s buscar
        
        Returns:
            Lista de movimentos
        """
        if not ETHERSCAN_API_KEY:
            logger.warning("Etherscan API key nÃ£o configurada")
            return []
                    ...
                    if data.get("status") == "1":
                        for tx in data.get("result", []):
                            try:
                                ...
                                # Estimar valor em USD (1 PAXG/XAUT â 1 oz gold)
                                gold_price = 2350  # Aproximado, seria obtido do coletor de preÃ§os
                                ...
                                if from_addr == "0x0000000000000000000000000000000000000000":
                                    ...
                                    # Verificar se Ã© depÃ³sito/saque de exchange
    ...
    def check_whale_alerts(self, movements: List[OnChainMovement]) -> List[OnChainMovement]:
        """
        Filtra movimentos que sÃ£o whale alerts.
    ...
    async def fetch_comex_warehouse(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            # Isso seria um PDF, precisaria de parsing especÃ­fico
            
            # Alternativa: scraping da pÃ¡gina de estatÃ­sticas
            url = "https://www.cmegroup.com/clearing/operations-and-deliveries/nymex-delivery-notices.html"
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
            }
            
            # Dados exemplo (em produÃ§Ã£o viria do scraping)
            warehouse_data = {
                "XAU": {
    ...
    async def fetch_shanghai_premium(self) -> Optional[Dict]:
        ...
        try:
            ...
            # Isso seria scraping do site chinÃªs
    ...
    def check_cot_alerts(self) -> List[Dict]:
        """
        Verifica se hÃ¡ alertas baseados no COT.
        ...
        for metal, cot in self.cot_data.items():
            ...
            # Alerta se mudanÃ§a semanal grande
...
def get_institutional_collector() -> InstitutionalCollector:
    """Retorna instÃ¢ncia singleton do coletor institucional."""
    global _collector
